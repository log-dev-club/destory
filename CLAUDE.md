...
├── services/        # 비즈니스 로직 (제목 해싱, Discord 알림 등)
...
### DB 스키마 (`api/migrations/20260913000000_init.sql`)
| 테이블 | 용도 | 주요 컬럼 |
|---|---|---|
| `users` | 작성자 | `user_name` (UNIQUE, 헤더 필터 키) |
| `posts` | 게시물 | `title`, `title_hash` (SHA-256, UNIQUE), `summary`, `content` (GFM 원문), `created_at` |
| `tags` / `post_tags` | 태그 (N:M) | `name` (UNIQUE) |
...
- 업로드 시 **제목을 해싱한 값을 DB에 저장** (URL 슬러그·중복 판별 등에 사용)
